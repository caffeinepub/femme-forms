...
export default function ArtworkDetailPage() {
  ...
  if (error || !artwork) {
    return (
      <ErrorState
        message={artwork === null ? 'Artwork not found' : 'Failed to load artwork'}
        onRetry={() => refetch()}
      />
    );
  }
  ...
  return (
    <div className="container py-12">
      <Button variant="ghost" onClick={() => navigate({ to: '/gallery' })} className="mb-6">
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <div className="overflow-hidden rounded-lg bg-muted shadow-sketch">
          {imageUrl ? (
            <img src={imageUrl} alt={artwork.title} className="h-full w-full object-contain" />
          ) : (
        ...
        <div className="space-y-6">
          <div>
            <div className="mb-3 flex items-start justify-between gap-4">
              <h1 className="font-display text-3xl font-bold tracking-tight sm:text-4xl">{artwork.title}</h1>
              <Badge variant={artwork.available ? 'default' : 'secondary'} className="shrink-0">
                {artwork.available ? 'Available' : 'Sold'}
              </Badge>
            </div>
            <p className="text-2xl font-semibold text-primary">{priceFormatted}</p>
          </div>
          ...
          {artwork.available && (
            <PurchaseInquiryForm itemId={artwork.id} itemType={ItemType.artwork} itemName={artwork.title} />
          )}
        </div>
      </div>
    </div>
